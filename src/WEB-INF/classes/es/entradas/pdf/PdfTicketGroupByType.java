 ...
 * @author takuya
 */
public class PdfTicketGroupByType extends HttpServlet implements Serializable {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            if(ticketTmp.getTicketData().getFechaInicio() == null){
                parametros.put("FECHA_INICIO", getServletContext().getRealPath(sdf.format(new Date())));
            } else{
                parametros.put("FECHA_INICIO", sdf.format(ticketTmp.getTicketData().getFechaInicio()));
            }
            if(ticketTmp.getTicketData().getFechaFin() == null){
                parametros.put("FECHA_FIN", getServletContext().getRealPath(sdf.format(new Date())));
            } else{
                parametros.put("FECHA_FIN", sdf.format(ticketTmp.getTicketData().getFechaFin()));
            }
            
            

            reporte = (JasperReport) JRLoader.loadObject(
                    getServletContext().getRealPath("/WEB-INF/classes/es/entradas/jasper/TicketGroupByType.jasper"));
