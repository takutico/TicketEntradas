...
public class Log {
    ...
    private Log() {
        // Se borran los ficheros de log de mas de 21 dias
        try {
            File logDir = new File(System.getProperty("user.dir") + File.separator + "logs");
            File[] logFiles = logDir.listFiles();
            if (logFiles != null) {
                Calendar calendarThreeWeekAgo = Calendar.getInstance();
                calendarThreeWeekAgo.add(Calendar.DAY_OF_MONTH, -21);
                for (int x = 0; x < logFiles.length; x++) {
                    ...
                    if (logFile.isFile()) {
                        ...
                        if (lastModifiedCalendar.before(calendarThreeWeekAgo)) {
                            logFile.delete();
                        }
                    }
                }
    ...
    public synchronized void log(String message, Throwable t, boolean sendIncident) {
        ...
        if (send2file) {
            ...
            try {
                ...
                if (sendIncident) {
                    //Utilidades.sendIncident(String mensaje);
                }
